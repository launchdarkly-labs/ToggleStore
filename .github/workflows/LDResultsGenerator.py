...
def _upgrade_inventory_sync_metric():
    ...
    check_url = f"https://app.launchdarkly.com/api/v2/metrics/{PROJECT_KEY}/{autogen_key}"
    ...
    flag_url = f"https://app.launchdarkly.com/api/v2/flags/{PROJECT_KEY}/{INVENTORY_SYNC_FLAG_KEY}"
    ...
    if resp.ok:
        ...
        logging.warning(f"Failed to create guarded rollout: {resp.status_code} {resp.text}")
        logging.info("Falling back to custom metric for guarded rollout...")
        rollout_payload["instructions"][1]["metrics"] = [
            {"metricKey": "telemetry-error-metric", "regressionThreshold": 0,
             "onRegression": {"rollback": True, "notify": True}}
        ]
        rollout_payload["instructions"][1]["metricKeys"] = ["telemetry-error-metric"]
        resp = requests.patch(rollout_url, headers=rollout_headers, json=rollout_payload)
        if resp.ok:
            logging.info("Guarded rollout created with fallback custom metric")
        else:
            logging.error(f"Fallback guarded rollout also failed: {resp.status_code} {resp.text}")
...
def inventory_sync_error_generator(client, stop_event):
    ...
    while retry_count < max_retries and not rollout_ready:
        ...
        if flag_details and is_measured_rollout(flag_details):
            rollout_ready = True
            logging.info("✅ Inventory Sync Upgrade rollout is ready!")
        else:
            retry_count += 1
            logging.info(f"Rollout not ready yet, retrying... ({retry_count}/{max_retries})")

    if not rollout_ready:
        logging.error("Inventory Sync Upgrade rollout failed to initialize after 30 seconds. Exiting.")
        return
...
def generate_results(project_key, api_key):
    ...
    try:
        ...
        inventory_sync_stop_event = threading.Event()
        ...
        inventory_sync_thread = threading.Thread(
            ...
            args=(client, inventory_sync_stop_event)
        )
