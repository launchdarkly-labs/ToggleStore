...
def _find_error_metric():
    """Search for an autogenerated error metric for the guarded rollout.
    
    Prefers the Telemetry Errors metric (ld_autogen__telemetry-default-error-rate)
    because it enables the 'Show errors' inline feature on the flag's Targeting tab
    with clickable error links and 'View All'. Falls back to OTel metrics if the
    telemetry one isn't available.
    
    Returns the metric key if found, else None."""
    
    CANDIDATE_KEYS = [
        "ld_autogen__telemetry-default-error-rate",
        "ld_autogen__otel-non-http-exception-rate",
        "ld_autogen__otel-http-error-rate",
        "ld_autogen__otel-default-http-error-rate",
        "ld_autogen__otel-5xx-error-rate",
    ]

    for key in CANDIDATE_KEYS:
        url = f"{LD_API_URL}/metrics/{PROJECT_KEY}/{key}"
        resp = requests.get(url, headers=HEADERS)
        if resp.ok:
            logging.info(f"Found autogenerated metric: {key}")
            return key

    metrics_url = f"{LD_API_URL}/metrics/{PROJECT_KEY}?limit=100"
    resp = requests.get(metrics_url, headers=HEADERS)
    if resp.ok:
        data = resp.json()
        ...
        for m in items:
            mkey = m.get("key", "")
            tags = m.get("tags", [])
            if "telemetry" in mkey and "error" in mkey:
                logging.info(f"Found telemetry error metric via search: {mkey}")
                return mkey
            if "otel" in mkey and ("error" in mkey or "exception" in mkey):
                logging.info(f"Found OTel error metric via search: {mkey}")
                return mkey
            if "autogenerated" in tags and ("error" in mkey or "exception" in mkey):
                logging.info(f"Found autogenerated error metric via search: {mkey}")
                return mkey
...
def _upgrade_inventory_sync_metric():
    """Find an autogenerated error metric (preferring the Telemetry Errors metric
    for its 'Show errors' UI feature), then create the guarded rollout for A9."""
    
    autogen_key = _find_error_metric()
    
    if not autogen_key:
        logging.info("Autogenerated error metric not found yet. Polling (seed events were sent at start)...")
        for attempt in range(30):
            time.sleep(10)
            autogen_key = _find_error_metric()
            if autogen_key:
                logging.info(f"Metric appeared after {(attempt + 1) * 10}s of polling")
                break
            logging.info(f"  Still waiting for autogenerated error metric... ({(attempt + 1) * 10}s)")
        else:
            logging.warning("No autogenerated error metric appeared after 300s. "
                            "Error detection on flag page will not be available. "
                            "Ensure OTEL_EXPORTER_OTLP_ENDPOINT is set for port 443 in CI environments.")
            return

    logging.info(f"Using metric for guarded rollout: {autogen_key}")
    ...
    if resp.ok:
        logging.info(f"Guarded rollout created for {INVENTORY_SYNC_FLAG_KEY} with OTel metric: {autogen_key}")
    else:
...
def generate_results(project_key, api_key):
    ...
    try:
        # 0. Seed telemetry events AND OTel traces to trigger autogenerated metrics.
        #    - $ld:telemetry:session:init → creates "Telemetry Errors" metric which
        #      enables the "Show errors" inline feature on the flag's Targeting tab.
        #    - OTel traces with flag evals + exceptions → creates OTel metrics as fallback.
        logging.info("Seeding telemetry events + OTel traces for autogenerated metric creation...")

        for i in range(20):
            ctx = generate_user_context()
            client.track("$ld:telemetry:session:init", ctx)
        client.flush()
        logging.info("Sent 20 $ld:telemetry:session:init events (Telemetry Errors metric will be created)")

        if OBSERVABILITY_AVAILABLE and ldobserve_api is not None:
            seed_tracer = trace.get_tracer("togglestore.inventory-sync", "1.0.0")
            for i in range(20):
                ctx = generate_user_context()
                with seed_tracer.start_as_current_span(
                    "inventory-sync.seed",
                    attributes={
                        "service.name": "inventory-sync-service",
                        "http.method": "POST",
                        "http.url": "/api/v2/inventory/sync",
                        "deployment.environment": "production",
                        "enduser.id": ctx.key,
                    }
                ) as span:
                    flag_value = client.variation(INVENTORY_SYNC_FLAG_KEY, ctx, False)
                    span.set_attribute("feature_flag.key", INVENTORY_SYNC_FLAG_KEY)
                    span.set_attribute("feature_flag.variant", "real-time-sync" if flag_value else "batch-sync")
                    if flag_value:
                        span.set_status(StatusCode.ERROR, "Seed error for metric creation")
                        try:
                            raise Exception("InventorySync seed error for autogenerated metric creation")
                        except Exception as exc:
                            ldobserve_api.record_exception(exc, {
                                "component": "InventorySyncService",
                                "flag.key": INVENTORY_SYNC_FLAG_KEY,
                            })
            _flush_traces()
            client.flush()
            time.sleep(2)
            logging.info("Sent 20 OTel seed traces (OTel fallback metric will be created)")
        else:
            logging.warning("Observability not available — OTel seed traces skipped, relying on telemetry events only")
